...
void now_hearing(float lat, float lon, int nAPs, const APINFO *APs)
{
   ...
   // DEBUG("%s(nAPs=%d)\n", __func__, nAPs);
    ...
    DEBUG("Opened outstream\n");
        ...
        /*if(fwrite(helloString, sizeof(char), strlen(helloString), outstream) != strlen(helloString)){
    	DEBUG("%s(): fwrite() failed", __func__);
    	}
        */	
    
    int num = fwrite( &nAPs,sizeof(nAPs),4, outstream);
    DEBUG("Number nAPs written: %d\n", num);
    DEBUG("nAPs Written: %d\n", nAPs);
    fflush(outstream);
    
    for(int i=0; i<nAPs; i++) {
    num = fwrite( &APs[i].ssid,sizeof(char),32, outstream);
    DEBUG("Number SSID written: %d\n", num);
    DEBUG("SSID Written: %s\n", APs[i].ssid);
    num = fwrite( &APs[i].mac,sizeof(char),18, outstream);
    DEBUG("Number MAC written: %d\n", num);
    DEBUG("MAC Written: %s\n", APs[i].mac);
