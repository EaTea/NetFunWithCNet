...
//byte order transformation taken from beej
...
//This is well send the information to the geoserver so that it can process the information 
...
void now_hearing(float lat, float lon, int nAPs, const APINFO *APs)
{
	...
	int NETWORKnAPs=htonl(sample.nAPs); // sends the number of access points as an int in one go. This need to change byte order
...
// This will given a direction the geoserver who will then request an image from the
...
void now_facing(FACING dir, int nAPs, const APINFO *APs)
{
	...
	if(!isGeoSet){ //Check if geoserver is set.
		set_popup("Your Geoserver is not");
		return;
	}
