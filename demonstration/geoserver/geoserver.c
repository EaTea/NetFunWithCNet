...
** Returns -2 if an internal error occurs
*/
int photo_getImageData (imageData *image, view *view) {
	...
	int sock_photo = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	if(sock_photo<0) {
		printf("socket() to obtain photo failed\n");
		return -2;
	}
	struct sockaddr_in addr_photo = prepSocketAddr(PHOTO_IP, PHOTO_PORT);
...
/* This function will accept a client connection .
...
void geoHearingProtocol(int clntSocket, const char *filename) {
	...
	processSamples(1, &sample);	 //triangulation processing of sample
...
void SendImage(int clntSocket){
	...
	if(k != 0){
		...
		//does this image exist in the cache? make it so (insert it into the cache) and
...
//Hoped to write samples to file when iPod finalized but could not due to extreme ineptness
...
The type def struct is defined in ipod3230.h
*/
...
int main(int argc, char *argv[]){
	...
	while(1) {		
		...
		switch(packettype)						 //Determine which protocol to call
		{
			case SAMPLE_DATA:
