...
*/
double distanceBetweenTwoLatLons		(
																		float, //latitude 1
																		float, //longitude 1
																		float, //latitude 2
																		float //longitude 2
...
float RSSIToDistance(int32_t rssi)
{
	...
	//conversion curve
	//return exp(fabsf((float)rssi)/100.0*5.0);
	if(rssi < 0) rssi *= -1;
	return fabsf((float)rssi);
}
...
double distanceBetweenTwoLatLons(float lat1, float lon1, float lat2, float lon2)
{
	//calculate pi manually
	double pi = atan(1)*4;

	//convert lats and lons to radii from equator and greenwich resp.
	double lat1_ra, lon1_ra, lat2_ra, lon2_ra;
	...
	double haverLat, haverLon, cosLat2, cosLat1;
	haverLat = haversine(lat2_ra-lat1_ra);
	haverLon = haversine(lon2_ra-lon1_ra);
	cosLat2 = cos(lat2_ra);
	cosLat1 = cos(lat1_ra);

	double sum = sqrt(haverLat + cosLat1*cosLat2*haverLon);
	double tmp = asin(sum);
	double distance = 2 * EARTH_RADIUS * tmp; //in metres
...
#ifdef VERBOSE
    PRINT_ERR("Origin at Lat: %f Lon: %f\n",lat_origin,lon_origin);
#endif
...
#ifdef VERBOSE
    PRINT_ERR("Different of Lat: %f Lon: %f\n",lat_diff,lon_diff);
#endif
}
